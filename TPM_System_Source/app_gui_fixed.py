...
def create_user():
    try:
        ...
        try:
            c.execute('''
                INSERT INTO users (username, password_hash, role, shift)
                VALUES (?, ?, ?, ?)
...
def update_user(user_id):
    try:
        ...
        if data.get('password'):
            ...
            c.execute('''
                UPDATE users
                SET username = ?, password_hash = ?, role = ?, shift = ?
                WHERE id = ?
...
def import_system_data():
    ...
    try:
        # Handle JSON data sent directly from frontend
        if request.is_json:
            import_data = request.get_json()
            if not import_data:
                return jsonify({'error': 'No data provided'}), 400
        else:
            # Handle file upload (fallback for direct file uploads)
